...
class SeguimientoPedidoProduccionImport implements ToCollection, WithHeadingRow
{
    public $procesadas = 0;
    public $vinculadas = 0;
    ...
    public function collection(Collection $rows)
    {
        DB::transaction(function () use ($rows) {
            foreach ($rows as $index => $row) {
                ...
                $pedidoRaw = strtoupper(trim((string) ($row['nro_pedido'] ?? $row['pedido'] ?? '')));
                $fechaPedidoRaw = $row['fecha_pedido'] ?? $row['fecha'] ?? null;
                $fechaPedido = $this->normalizarFecha($fechaPedidoRaw);

                if ($nroOt === '' || $pedidoRaw === '') {
                    $this->omitidas[] = $index + 2;
                    continue;
                }

                $nroPedido = preg_match('/^P/i', $pedidoRaw)
                    ? 'P' . ltrim(substr($pedidoRaw, 1))
                    : 'P' . $pedidoRaw;
